...
$('#product').change(function () {
    ...
    $.get('/get-product-detail/' + product_id, function (data) {
        $('#product-price').html(data.price);
        $('#product-name').html(data.name);
        $('#image-product').attr('src', data.img);
    })
...
$('#location').change(function () {
    ...
    $('#registration-location').html(pro_name);
})

function calc() {

    let price = parseInt($('#product').val());
    ...
    let total = price + tax + plate + 340000 + 1560000 + 480000;

    $('#result').html("Tổng lăn bánh: " + total.toLocaleString() + " VNĐ");

}

$('.contact-form-style-1').on('submit', function(e){
    e.preventDefault();
    ...
    $.post($(form).attr('action'), $(form).serialize(), function(res){
        ...
        if(res.error === false){
            form.reset();
            window.location.href = '/gui-thong-tin-thanh-cong';
        }
    });
});